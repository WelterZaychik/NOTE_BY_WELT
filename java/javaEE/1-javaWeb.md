...
![BS结构系统的通信原理2](ph/javaweb/BS结构系统的通信原理2.png)
...
- 思考：我们自己new的Servlet对象受WEB容器的管理吗？
  ...
  - ![web容器中的map集合](ph/javaweb/web容器中的map集合.jpg)
...
- 注册驱动的作用就是通知Java程序我们即将连接的事那个品牌的数据库

  ``````JAVA
  	Driver driver = new com.mysql.Driver();
  	//这个是老版本的，现在新版mysql修正了的路径
  	Driver driver = new com.mysql.cj.jdbc.Driver();
  	//这些都不是最终的方法，因为要写这么多东西，开发的时候及其不方便
  
  	//这个时候我们直接使用之前学的静态代码块的知识,直接获取类名即可注册
  	//同时，这样的方法还可以直接将字符串写在配置文件中，更加方便
  	Class.forName("com.mysql.cj.jdbc.Driver");
  ``````

#### 第二步，获取连接：

- Java进程和mysql进程，两个进程的通道开启【注意这个时候两个进程的位置可能不在一个地方，一个北京一个伤害都有可能】,获取连接其实就是实现Collection接口
  ...
  - 资源名：
  	这个服务在的某个资源。
  	jdbc:mysql://		这是Java程序和mysql之间通信的协议
  	localhost:			这是本机IP地址，本机IP地址还可以写成：127.0.0.1
  	3306				mysql数据库的端口号
  	helloworld			mysql数据库的名称
  	
  - 如果是Oracle数据库的话就是：
  	Oracle:jdbc:thin:@localhost:1521:helloworld
  ``````

#### 第三步，获取数据库操作对象：

- 这个对象很重要，用这个对象执行sql语句

  ``````JAVA
  	Statement stmt = conn.createStatement();
  	//通过一个链接对象Connection可以创建多个Statement对象
  	Statement stmt2 = conn.createStatment();
  
  	//Statement接口存在一个子类PreparedStatent
  	//此时就不是获取数据库操作对象了是获取预编译的数据库操作对象
  	 PreparedStatement stmt3 = conn.prepareStatement(sql);
  	 //后面的语句也要有相应的更改，需要传值。
  	 
  - Statement:【先进行字符串拼接，然后再进行sql语句的编译。】
  	-  优点：使用Statement可以进行sql语句的拼接
  	 * 缺点：因为拼接的存在，导致可能给不法分子机会。导致sql注入
  * PreparedStatement:【先进行sql语句的编译，然后再进行sql语句的传值】
  	- 优点：避免sql注入
...
- 执行CRUD
  ...
  ``````JAVA
  	...
  	//这个Statement接口中的executeUpdate方法是钻门来执行DML语句的。
  	//返回值表示是影响了数据库表中的总记录条数。
  
  	String sql = "select ....";
  	ResultSet rs = stemt.executeQuery(sql);
  	//executeQuery是来执行DQL语句的方法，ResultSet就是查询结果集对象
  
  	//如果第三步使用的是PreparedStatement,那么这里就不需要传sql语句了，直接把括号空起来即可
  	ResultSet rs = stemt.executeQuery();
  ``````

#### 第五步，查询结果集：

- 必须要有ResultSet才可以调用【也就是说在第四步中使用的sql语句是select语句，才有这一步】

  ``````JAVA
  	//ResultSet解控中有相应的方法来遍历结果集
  	boolean ResultSet.next();//指针向前移动一位，判断是否有值
  
  	XXX getXXX();//这个方法就是读取查询结果，XXX表示查询的结果类型
  	int sal = getInt();//获取int类型的数据
  	.......
  	String sal = getString();//这个方法可以获取任意类型的数据，将其他数据类型转换成String字符串然后返回。 
  ``````

#### 第六步，释放资源：

- 关闭所有资源通道，就和关闭流一样【因为JDBC毕竟是进程之间的同性，占用很多资源，需要关闭】

  //释放资源的时候有一定的顺序，先释放ResultSet，在释放Statement，最后释放Connection。
  	//当然要分别进行try...catch和if null处理
  	//应该放到finally语句中，确保会执行。
  	finally{
  		if (stmt != null){
  			try{
  				stmt.close();	
...
- JDBC默认情况下支持自动提交，只要执行一次DML语句就自动提交一次

- 在事迹开发中必须将JDBC的自动提交机制关闭到，改成手动提交
  当一个完整的事务结束之后再提交
  - conn.setAutoCommit(false); 该方法默认是true，表示默认自动提交机制
  - conn.commit();手动提交
  - conn.rollback();手动回滚

## 总结

- 到这里JDBC六步就结束了，只是略微了解了六步的功能，详细代码在

  [JDBC详细代码](JDBC详细代码.md)